  ...
  return async function uploadAttachment(filePaths, threadId, type = MessageType.DirectMessage, isUseProphylactic = false) {
    ...
    for (const filePath of filePaths) {
      ...
      const processUpload = async () => {
          ...
          while (retryCount < maxRetries && !success) {
            try {
              ...
              if (retryCount < maxRetries) {
                ...
                console.error(`Chunk ${chunkId} upload failed after ${maxRetries} attempts, skipping...`);
                return null; // Trả về null thay vì throw để không crash Promise.all
              }
            }
          }
        };
        ...
        const validResults = uploadedResults.filter(r => r != null);
        if (validResults.length === 0) {
          throw new ZaloApiError("Upload failed: No chunks uploaded successfully.");
        }
      ...
      try {
        ...
        try {
          ...
          console.error("Lỗi upload lần 2, bỏ qua file này:", retryError);
          // Không crash bot, tiếp tục với file tiếp theo
          results.push({ error: true, message: retryError.message, filePath });
        }
      }
    }

    // Nếu tất cả đều lỗi, ném lỗi để caller xử lý
    if (results.every(r => r.error)) {
      throw new ZaloApiError("Tất cả file đều upload thất bại");
    }
    return results;
  };
}