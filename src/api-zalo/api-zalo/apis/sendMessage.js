...
export function sendMessageFactory(api) {
  ...
  async function handleMessage({ msg, mentions, quote, style, styles }, threadId, type, ttl) {
    let finalStyle = style || styles;
    if (Array.isArray(finalStyle)) {
      // Chuyển đổi định dạng custom (c_..., s_..., b) sang định dạng Zalo chuẩn
      const normalizedStyles = finalStyle.map(s => {
        const item = { start: s.start, len: s.len };
        const typeStr = String(s.type || s.st || "");
        
        if (typeStr === "b" || typeStr === "bold") {
            item.type = "font";
            item.value = "bold";
        } else if (typeStr === "i" || typeStr === "italic") {
            item.type = "font";
            item.value = "italic";
        } else if (typeStr.startsWith("c_")) {
            item.type = "color";
            item.value = "#" + typeStr.slice(2);
        } else if (typeStr.startsWith("s_")) {
            item.type = "size";
            item.value = typeStr.slice(2);
        } else {
            item.type = s.type;
      ...
      finalStyle = JSON.stringify(normalizedStyles);
    }
    style = finalStyle;
    ...
    const isPlainObject = quote && typeof quote === "object" && !isValidInstance;
    if (quote && !isValidInstance && !isPlainObject) throw new ZaloApiError("Invalid quote message");
  ...
  async function handleAttachment({ msg, attachments, mentions, quote, isUseProphylactic }, threadId, type, ttl) {
  ...
  return async function sendMessage(message, threadId, type = MessageType.DirectMessage) {
    ...
    if (attachments && attachments.length > 0) {
      ...
      const handledData = await handleAttachment({ msg, mentions, attachments, quote, isUseProphylactic }, threadId, type, ttl);
    ...
    // --- TỰ ĐỘNG THU HỒI (AUTO-UNDO) ---
    if (ttl > 0) {
      // Nếu ttl nhỏ (ví dụ < 86400), coi như là giây, ngược lại là mili giây
      const delay = ttl < 86400 ? ttl * 1000 : ttl;
      
      const scheduleUndo = (res) => {
        if (!res || !res.msgId) return;
        setTimeout(async () => {
          try {
            ...
            if (typeof undoFn === "function") {
              await undoFn({
                type: type,
                threadId: threadId,
                data: { quote: { globalMsgId: String(res.msgId), cliMsgId: String(res.cliMsgId || Date.now()) } }
              });
            }
          } catch (e) {
            console.error("[AUTO-UNDO ERROR]", e.message);
          }
        }, delay);
      };

      if (responses.message) scheduleUndo(responses.message);
      if (responses.link) scheduleUndo(responses.link);
      if (Array.isArray(responses.attachment)) {
        responses.attachment.forEach(att => scheduleUndo(att));
      }
    }
