...
export async function getImageMetaData(filePath) {
  ...
  // Một số file có thể khiến sharp không đọc được metadata (định dạng lạ / file hỏng).
  // Không được throw để tránh làm hỏng flow upload.
  try {
    ...
    try {
      // Fallback: ép convert về PNG buffer rồi đọc lại metadata
      const pngBuffer = await sharp(fileData).png().toBuffer();
      const imageData = await sharp(pngBuffer).metadata();
      return {
        fileName,
        totalSize: imageData.size,
        width: imageData.width,
      ...
      // Fallback cuối: trả metadata tối thiểu để upload không bị crash
      return {
        fileName,
...
export function getVideoMetadata(filePath) {
  return new Promise((resolve, reject) => {
    ...
    if (typeof filePath !== 'string') {
      reject(new Error(`filePath must be a string, received ${typeof filePath}`));
      return;
    }
